...
class SypexDumper
{
    ...
    public function __construct()
    {
        ...
        $this->name = 'Sypex Dumper 2.0.11 for Cotonti Siena 0.9.18 (php7)';
    }
    ...
    public function connect($host = null, $port = null, $user = null, $pass = null)
    {
        ...
        $this->mysqli_link = mysqli_connect($this->CFG['my_host'], $this->CFG['my_user'], $this->CFG['my_pass']);
        if ($this->mysqli_link) {
            if (V_PHP > 50202) {
                mysqli_set_charset($this->mysqli_link, 'utf8') or sxd_my_error();
            } else {
                mysqli_query($this->mysqli_link, 'SET NAMES utf8') or sxd_my_error();
            }
            define('V_MYSQL', sxd_ver2int(mysqli_get_server_info($this->mysqli_link)));
        } else {
            define('V_MYSQL', 0);

//            $this->error = 'sxd.actions.tab_connects();alert(' . sxd_esc($this->mysqli_link->mysqli_error()) . ');';
    ...
    {
        $r = mysqli_query($this->mysqli_link, 'DROP DATABASE `' . sxd_esc($name, false) . '`') or sxd_my_error();
        if ($r) {
    ...
    {
        $r = mysqli_query($this->mysqli_link, 'CREATE DATABASE `' . sxd_esc($req['name'], false) . '`' . (V_MYSQL > 40100 ? "CHARACTER SET {$req['charset']} COLLATE {$req['collate']}" : ''));
        if ($r) {
    ...
    public function runServices($job)
    {
        ...
        if (isset($serv[ $job['type'] ])) {
            mysqli_select_db($this->mysqli_link, $job['db']);
            $filter = $object = [];
            $this->createFilters($job['obj'], $filter, $object);
            $r = mysqli_query($this->mysqli_link, 'SHOW TABLE STATUS') or sxd_my_error();
            if (!$r) {
                return;
            }
            $tables = [];
            ...
            $r = mysqli_query($this->mysqli_link, $sql) or sxd_my_error();
            if (!$r) {
                return;
            }
            $res = [];
    ...
    public function runRestoreJob($continue = false)
    {
        ...
        mysqli_select_db($this->mysqli_link, $this->JOB['db']);
        if (is_null($this->JOB['obj'])) {
        ...
        if ($this->JOB['strategy'] > 0) {
            $r = mysqli_query($this->mysqli_link, 'SHOW TABLES') or sxd_my_error();
            while ($item = mysqli_fetch_row($r)) {
        ...
        //$enable_index = [];
        if (V_MYSQL > 40014) {
            mysqli_query($this->mysqli_link, 'SET UNIQUE_CHECKS=0');
            mysqli_query($this->mysqli_link, 'SET FOREIGN_KEY_CHECKS=0');
            if (V_MYSQL > 40101) {
                mysqli_query($this->mysqli_link, "SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO'");
            }
            if (V_MYSQL > 40111) {
                mysqli_query($this->mysqli_link, 'SET SQL_NOTES=0');
            }
        }
        $log_sql   = false;
        $fields    = '';
        ...
        while ($q = sxd_read_sql($this->fh_tmp, $seek, $ei)) {
            ...
            switch ($q{0}) {
                ...
                case 'I':
                    if (preg_match('/^INSERT( INTO `(.+?)`) VALUES/', $q, $m)) {
                        ...
                        //mysqli_query($this->mysqli_link, "LOCK TABLES `{$tab}` WRITE") or die (mysqli_error());
                        mysqli_query($this->mysqli_link, "ALTER TABLE `{$tab}` DISABLE KEYS") or sxd_my_error();
                        //if(!empty($this->JOB['autoinc'])) mysqli_query($this->mysqli_link, "ALTER TABLE `{$tab}` AUTO_INCREMENT = 1") or sxd_my_error();
                        $ex = 1;
                    }
                    break;
                case 'C':
                    ...
                    if (preg_match("/\#\t(TC|TD|VI|PR|FU|TR|EV)`(.+?)`(([^_]+?)_.+?)?$/", $q, $m)) {
                        ...
                        if ($m[1] == 'TC') {
                            ...
                            if ($tc && ($this->JOB['strategy'] == 0 || isset($tab_exists[ $m[2] ]))) {
                                mysqli_query($this->mysqli_link, "{$tc} `{$m[2]}`") or sxd_my_error();
                            }
                        } elseif ($m[1] == 'TD') {
                            $ei = 1;
                        } else {
                            ...
                            $this->rtl[7] = 0;
                            $this->rtl[8] = 0;
                            mysqli_query($this->mysqli_link, "DROP {$types[$m[1]]} IF EXISTS `{$m[2]}`") or sxd_my_error();
            ...
            if ($ex) {
                ...
                $this->rtl[1] = time();
                fwrite($this->fh_rtl, implode("\t", $this->rtl));
                if (mysqli_query($this->mysqli_link, $q)) {
                    if ($insert) {
                        $c = 1;
                    }
                } else {
                ...
                if ($c) {
                    $i            = mysqli_affected_rows($this->mysqli_link);
                    $this->rtl[3] = ftell($this->fh_tmp) - $seek;
                    $this->rtl[7] += $i;
                    $this->rtl[10] += $i;
                    fseek($this->fh_rtl, 0);
        ...
        foreach ($this->JOB['todo']['TA'] as $tab) {
            ...
            mysqli_query($this->mysqli_link, "ALTER TABLE `{$tab[1]}` ENABLE KEYS") or sxd_my_error();
            $this->rtl[1] = time();
            $this->rtl[5] = $tab[1];
            fseek($this->fh_rtl, 0);
            fwrite($this->fh_rtl, implode("\t", $this->rtl));
                ...
                switch ($q{0}) {
                    ...
                    case 'C':
                        ...
                        if (preg_match('/^CREATE TABLE.+?`(.+?)`/', $q, $m)) {
                            ...
                            //mysqli_query($this->mysqli_link, "DROP TABLE IF EXISTS `{$tab}`");
                            if ((!empty($this->JOB['correct']) && !empty($this->JOB['charset']))) {
                                $q = preg_replace('/(DEFAULT)?\s*(CHARSET|CHARACTER SET|COLLATE)[=\s]+\w+/i', '', $q) . (V_MYSQL < 40100 ? '' : ' DEFAULT CHARSET=' . $this->JOB['charset']);
    ...
    public function addBackupJob($job)
    {
        ...
        foreach ($queries as $query) {
            ...
            $r = mysqli_query($this->mysqli_link, 'SHOW ' . $query[0]) or sxd_my_error();
            if (!$r) {
                continue;
            }
            $todo[ $t ]   = [];
        ...
        if (V_MYSQL > 50014 && (!empty($object['VI']) || !empty($filter['VI']))) {
            ...
            $r = mysqli_query($this->mysqli_link, "SELECT table_name, view_definition /*!50121 , collation_connection */ FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = '{$this->JOB['db']}'") or sxd_my_error();
    ...
    public function runBackupJob($continue = false)
    {
        ...
        mysqli_query($this->mysqli_link, 'SET SQL_QUOTE_SHOW_CREATE = 1');
        $types  = ['VI' => 'View', 'PR' => 'Procedure', 'FU' => 'Function', 'TR' => 'Trigger', 'EV' => 'Event'];
        $fcache = '';
        $writes = 0;

        if (V_MYSQL > 40101) {
            mysqli_query($this->mysqli_link, "SET SESSION character_set_results = '" . ($this->JOB['charset'] ? $this->JOB['charset'] : 'binary') . "'") or sxd_my_error();
        }
        $time_old  = time();
        $exit_time = $time_old + $this->CFG['time_web'] - 1;
                ...
                switch ($n[0]) {
                    ...
                    case 'TA':
                        ...
                        if ($n[0] == 'TC' || $this->rtl[7] == 0) {
                            ...
                            $r = mysqli_query($this->mysqli_link, "SHOW CREATE TABLE `{$n[1]}`") or sxd_my_error();
                        ...
                        $notNum = [];
                        $r = mysqli_query($this->mysqli_link, "SHOW COLUMNS FROM `{$n[1]}`") or sxd_my_error();
                        $fields = 0;
                        ...
                        $z        = 0;
                        // Достаем данные
                        $r = mysqli_query($this->mysqli_link, "SELECT {$no_cache}* FROM `{$n[1]}`{$from}", MYSQLI_USE_RESULT);
                        while ($row = mysqli_fetch_row($r)) {
                            ...
                            for ($k = 0; $k < $fields; ++$k) {
                                if (!isset($row[ $k ])) {
                                    ...
                                    $row[ $k ] = '\'' . mysqli_real_escape_string($this->mysqli_link, $row[ $k ]) . '\'';
                    ...
                    default:
                        if (V_MYSQL < 50121 && $n[0] == 'TR') {
                            ...
                            $r = mysqli_query($this->mysqli_link, "SELECT * FROM `INFORMATION_SCHEMA`.`TRIGGERS` WHERE `TRIGGER_SCHEMA` = '{$this->JOB['db']}' AND `TRIGGER_NAME` = '{$n[1]}'") or sxd_my_error();
                            $item = mysqli_fetch_assoc($r);
                            ...
                            $r = mysqli_query($this->mysqli_link, "SHOW CREATE {$types[$n[0]]} `{$n[1]}`") or sxd_my_error();
                            $item = mysqli_fetch_assoc($r);
    ...
    public function setNames($collation)
    {
        ...
        if ($this->rtl[6] != $collation) {
            mysqli_query($this->mysqli_link, 'SET NAMES \'' . preg_replace('/^(\w+?)_/', '\\1\' COLLATE \'\\1_', $collation) . '\'') or sxd_my_error();
            /*if(!$this->rtl[7])*/
    ...
    public function getDBList()
    {
        ...
        if ($this->CFG['my_db']) {
            ...
            $result = mysqli_query($this->mysqli_link, 'SHOW DATABASES') or sxd_my_error();
        ...
        if (V_MYSQL < 50000) {
            foreach ($items as $item) {
                $tables = mysqli_query($this->mysqli_link, "SHOW TABLES FROM `{$item}`") or sxd_my_error();
                if ($tables) {
            ...
            $result = mysqli_query($this->mysqli_link, "SELECT `table_schema`, COUNT(*) FROM `information_schema`.`tables` {$where} GROUP BY `table_schema`") or sxd_my_error();
            while ($item = mysqli_fetch_row($result)) {
                if ($item[0] == 'information_schema' || $item[0] == 'mysql' || $item[0] == 'performance_schema') {
                    continue 1;
                }
    ...
    public function getCharsetList()
    {
        ...
        if (V_MYSQL > 40101) {
            ...
            $r = mysqli_query($this->mysqli_link, 'SHOW CHARACTER SET') or sxd_my_error();
            if ($r) {
                while ($item = mysqli_fetch_assoc($r)) {
    ...
    public function getCollationList()
    {
        ...
        if (V_MYSQL > 40101) {
            ...
            $r = mysqli_query($this->mysqli_link, 'SHOW COLLATION') or sxd_my_error();
            if ($r) {
                while ($item = mysqli_fetch_assoc($r)) {
                    if (empty($def_charsets) || preg_match($def_charsets, $item['Charset'])) {
    ...
    {
        mysqli_select_db($this->mysqli_link, $db_name);
        // Достаем таблицы
        $r               = mysqli_query($this->mysqli_link, 'SHOW TABLE STATUS');
        ...
        if ($r) {
            ...
            if (V_MYSQL > 50014 && $tree != 'services') {
                ...
                for ($i = 0, $l = count($shows); $i < $l; ++$i) {
                    $r = mysqli_query($this->mysqli_link, 'SHOW ' . $shows[ $i ]);
...
{
   global $SXD; 
    trigger_error(mysqli_error($SXD->mysqli_link), E_USER_ERROR);
}
